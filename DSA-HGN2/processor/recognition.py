...
class FT_Processor(Processor):
    ...
    Updated: Uses Entropy Regularization with Curriculum Strategy.
    """
    ...
    def train(self, epoch):
        ...
        # =========================================================================
        # [PHASE 1 OPTIMIZATION] Curriculum Entropy Strategy
        # 目标: 先学特征(Dense)，后修剪结构(Sparse)，防止梯度消失
        # =========================================================================
        target_lambda_entropy = 0.5  # 最终目标值
        start_curriculum = 20  # 开始增加权重的 Epoch
        end_curriculum = 40  # 达到最大权重的 Epoch

        if epoch <= start_curriculum:
            current_lambda = 0.0
        elif epoch < end_curriculum:
            # 线性增加
            ratio = (epoch - start_curriculum) / (end_curriculum - start_curriculum)
            current_lambda = target_lambda_entropy * ratio
        else:
            current_lambda = target_lambda_entropy

        # 动态更新参数
        self.arg.lambda_entropy = current_lambda

        # 仅在每个 Epoch 开始时打印一次策略状态
        self.io.print_log(f'[Curriculum] Epoch {epoch}: Dynamic lambda_entropy set to {current_lambda:.4f}')
        # =========================================================================
        ...
        for data, label in loader:
            ...
            # Total Loss
            # 使用动态调整后的 self.arg.lambda_entropy
