...
class Processor(IO):
    ...
    def train(self, epoch):
        ...
        for batch_idx, (data, label, index, gt_joints_raw) in enumerate(loader):
            ...
            # GT Raw from Feeder: (N, 3, T, V, M)
            # Model combines batch(N) and person(M) dimensions -> (N*M, ...)
            # We need to reshape GT to match predicted_joints: (N*M, 3, T, V)
            ...
            # 4.2 回归损失 (Regression Loss) - 约束特征保留空间结构
            # 确保预测值和GT维度一致
            if predicted_joints.shape != gt_joints.shape:
                # 简单的形状保护，理论上不应触发
                print(f"Warning: Shape mismatch. Pred: {predicted_joints.shape}, GT: {gt_joints.shape}")
                predicted_joints = F.interpolate(predicted_joints, size=(gt_joints.size(2), gt_joints.size(3)))

            L_reg = nn.MSELoss()(predicted_joints, gt_joints)

            # 4.3 骨骼方向损失 (Bone Direction Loss) - 约束几何一致性
            L_bone_cos = torch.tensor(0.0, device=self.dev)

            # 尝试从模型 graph 中获取骨骼连接定义
            ...
            loss = L_cls + lambda_reg * L_reg + lambda_bone * L_bone_cos
