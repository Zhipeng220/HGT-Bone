...
class PhysicallyGuidedDSAHypergraph(nn.Module):
    ...
    def __init__(self, in_channels, num_dynamic_edges=8, ratio=8, use_virtual_conn=True, num_point=21, **kwargs):
        ...
        print(f"[INIT] PhysicallyGuidedDSAHypergraph. Dynamic Edges={num_dynamic_edges}, Physical Edges=6")

    def _create_finger_masks(self):
        """
        Creates masks based on HGT-Bone anatomical grouping.
        """
        # 1. 增加掩码数量到 6 (5根手指 + 1个指尖交互)
        masks = torch.zeros(6, self.num_point)

        # 2. 定义每根手指内部的节点索引 (Intra-finger)
        # 这些索引在 Joint(22) 和 Bone(21) 模式下通常不需要变动，
        # 因为前21个节点的相对顺序在两种模式下往往是一致的(除了Root偏移)
        fingers_indices = [
        ...
        # ==========================================
        # [插入位置] 自动适配 Joint/Bone 及其 Motion 流
        # ==========================================
        if self.num_point == 22: # Joint Stream & J-Motion
            # SHREC'17 22关节定义: 0:Wrist, 1:Palm,
            # 指尖索引: 5(Thumb), 9(Index), 13(Middle), 17(Ring), 21(Pinky)
            tip_indices = [5, 9, 13, 17, 21]
        else: # Bone Stream & B-Motion (通常为21节点)
            # Bone定义: 0:Wrist-Palm
            # 指尖骨骼索引: 4(Thumb), 8(Index), 12(Middle), 16(Ring), 20(Pinky)
            tip_indices = [4, 8, 12, 16, 20]
        # ==========================================

        # 3. 填充第6个掩码 (指尖交互 Inter-finger)
        valid_tips = [idx for idx in tip_indices if idx < self.num_point]
        if valid_tips:
            masks[5, valid_tips] = 1.0
