...
class FT_Processor(Processor):
    ...
    def train(self, epoch):
        ...
        for data, label, index, gt_joints_raw in loader:
            ...
            # [FIXED] 自动对齐 V 维度 (处理 21 bones vs 22 joints 的情况)
            if predicted_joints.shape[-1] != gt_joints.shape[-1]:
                predicted_joints_aligned = F.interpolate(predicted_joints, size=(gt_joints.size(2), gt_joints.size(3)),
                                                         mode='bilinear', align_corners=False)
            else:
                predicted_joints_aligned = predicted_joints

            loss_reg = nn.MSELoss()(predicted_joints_aligned, gt_joints)
            ...
            # [FIXED] 优先使用 source_edges (关节连接)，否则回退到 edge
            edges = []
            ...
            if edges and len(edges) > 0:
                try:
                    ...
                    # 确保索引不越界 (针对对齐后的预测结果)
                    if max(src_idx + dst_idx) < predicted_joints_aligned.shape[-1]:
                        pred_vec = predicted_joints_aligned[..., src_idx] - predicted_joints_aligned[..., dst_idx]
            ...
            # -----------------------------------------------------------
            # Weights for new losses (Can be tuned in yaml eventually)
            lambda_reg = 0.1
            lambda_bone = 0.05
